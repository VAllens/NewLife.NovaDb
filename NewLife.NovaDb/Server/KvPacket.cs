...
internal static class KvPacket
{
    ...
    {
        var tableBytes = (tableName ?? "default").ToPooledUtf8Bytes();
        var keyBytes = key.ToPooledUtf8Bytes();
        try
        {
    ...
    {
        var tableBytes = (tableName ?? "default").ToPooledUtf8Bytes();
        var patternBytes = pattern.ToPooledUtf8Bytes();
        try
        {
    ...
    public static IPacket EncodeGetAll(String tableName, String[] keys)
    {
        ...
        {
            using var pooledKeyBytes = key.ToPooledUtf8Bytes();
